...
typedef struct
{
    U_FloatData Rpm_left;
    U_FloatData Rpm_right;
}S_ReceiveData;
...
//------------------------------  接受驱动板数据  ------------------------------
    ...
    {
        Serial2.readBytes(RecBuff,10);
        for(i = 0;i < 4;i++)
        {
            RecData.Rpm_left.data[i] = RecBuff[i];
            RecData.Rpm_right.data[i] = RecBuff[i+4];
        }

        sprintf(printbuff, "Rpm_left = %.3f", RecData.Rpm_left.val);
        Serial3.println(printbuff);
        sprintf(printbuff, "Rpm_right = %.3f", RecData.Rpm_right.val);
        Serial3.println(printbuff);
...
//Send the speed of two Wheel and the steer angular
void moveBase()
{
    u8 i;
    ...
    motor1_controller.spin(motor1_pid.compute(req_rpm.motor1, current_rpm1));//2
    motor2_controller.spin(motor2_pid.compute(req_rpm.motor2, current_rpm2));//3
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
    if(kinematics.base_platform == Kinematics::ACKERMANN || kinematics.base_platform == Kinematics::ACKERMANN1)
    {
        ...
        current_rpm1 = RecData.Rpm_left.val;        //得到当前两后轮转速 r/min
        current_rpm2 = RecData.Rpm_right.val;
        current_vel = kinematics.getVelocities(current_steering_angle, current_rpm1, current_rpm2);

//------------------------------  发送数据至驱动板  ------------------------------     
        current_steering_angle = mapFloat(current_steering_angle, -MAX_STEERING_ANGLE, MAX_STEERING_ANGLE, PI, 0) * (180 / PI)-90;
        SendData.Angle.val = current_steering_angle;
        SendData.Rpm_left.val = req_rpm.motor1;
        SendData.Rpm_right.val = req_rpm.motor2;
        for(i = 0;i < 4;i++)
        {
            SendBuff[i] = SendData.Angle.data[i];
            SendBuff[i+4] = SendData.Rpm_left.data[i];
            SendBuff[i+8] = SendData.Rpm_right.data[i];
        }
        SendBuff[12] = '\r';
        SendBuff[13] = '\n';
        Serial2.write(SendBuff, 14);
//-----------------------------------------------------------------------------
    }
    else
    {
...
void printDebug()
{
    ...
    current_steering_angle = mapFloat(current_steering_angle, -MAX_STEERING_ANGLE, MAX_STEERING_ANGLE, PI, 0) * (180 / PI)-90;
    sprintf (buffer, "Encoder FrontLeft  : %f", RecData.Rpm_left.val);//motor1_encoder.read());
    nh.loginfo(buffer);
    sprintf (buffer, "Encoder FrontRight : %f", RecData.Rpm_right.val);//motor2_encoder.read());
    nh.loginfo(buffer);
    sprintf (buffer, "current_steering_angle: %f", current_steering_angle);
    nh.loginfo(buffer);
